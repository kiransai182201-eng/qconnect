-- Migration: Add payment_method column and update place_secure_order function to accept p_payment_method
ALTER TABLE public.orders ADD COLUMN IF NOT EXISTS payment_method TEXT DEFAULT 'Pay After Meal';
...
BEGIN
    -- 1. Validate availability of ALL items first (double-check before order)
    ...
    -- 2. If any items are unavailable, return error response (not an exception)
    ...
    -- 4. Calculate total amount by looking up actual prices
    FOR v_item IN SELECT * FROM jsonb_to_recordset(p_cart_items) AS x(item_id UUID, quantity INT)
    LOOP
    ...
    -- 5. Insert the order including payment_method
